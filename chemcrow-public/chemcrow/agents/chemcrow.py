...
def _make_llm(model, temp, api_key, streaming: bool = False):
    if model.startswith("gpt-3.5-turbo") or model.startswith("gpt-4") or model.startswith("gpt-4o"):
...
class ChemCrow:
    ...
    def run(self, prompt, image_url: Optional[str] = None):
        # Prepare the input for the agent
        inputs = {"input": prompt}
        if image_url:
            inputs["image_url"] = image_url
        # Run the agent executor with the provided inputs
        outputs = self.agent_executor(inputs)
        return outputs["output"]
